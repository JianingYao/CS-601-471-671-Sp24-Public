...
class CustomModelforSequenceClassification(nn.Module):
    ...
    def __init__(self, model_name, num_labels=2, type="full"):
        ...
        # modify device setting (using the device of input_ids)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        if self.type == "full":
            ...
            last_hidden_output_mean = last_hidden_output.mean(axis=1)
            # pass the mean into the self.classifier to get the logits
            logits = self.classifier(last_hidden_output_mean)
            # your code ends here
            ...
            # the prefix is at self.prefix, but this is only one prefix,
            # we want to append it to each instance in a batch
            # we make multiple copies of self.prefix here. the number of copies = batch size
            self.prefix = self.prefix.to(input_ids.device)
            batch_prefix = torch.stack([self.prefix] * input_ids.shape[0], dim=0)
            # concatenate the input embeddings and our prefix, make sure to put them into our gpu
            ...
            inputs_embeds = torch.cat((batch_prefix, input_embeddings), axis=1)
            ...
            attention_mask = torch.cat((torch.ones(input_ids.shape[0], prefix_length).to(input_embeddings.device), attention_mask), axis=1)
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, test_dataloder, device, lr, model_name):
    ...
    if mymodel.type == "head":
        ...
        # you also need to get the parameters of the classifier (head),
        # you can do this by calling mymodel.head.parameters()
