...
def main():
    ...
    if data_args.validation_file is not None:
        raw_datasets["validation"] = Dataset.from_generator(dataset_gen,
                                                            gen_kwargs={"filepath": data_args.validation_file})
    ...
    def preprocess_function(examples):
        ...
        model_inputs = {
            'input_ids': tokenizer(inputs, max_length=data_args.max_source_length, padding=padding, truncation=True)['input_ids']
        }
        ...
        labels = {
            'input_ids': tokenizer(text_target=targets, max_length=max_target_length, padding=padding, truncation=True)['input_ids']
        }
        # your code ends here
    ...
    if training_args.do_predict:
        ...
        if model_args.do_sample:
            ...
            predict_results = trainer.predict(
                predict_dataset, metric_key_prefix="predict", max_length=max_length, do_sample=True, temperature=0.7,
                top_k=20
            )
        else:
            predict_results = trainer.predict(
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                predictions = np.where(predict_results.predictions != -100, predict_results.predictions,
